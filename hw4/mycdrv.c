...
{	
	...
	else if(curr_dev->seek_dir == REVERSE)
	{
		...
		rstring = (char *)(kmalloc(bytes_to_do*sizeof(char),GFP_KERNEL));

		for(i=0;i<bytes_to_do;i++)
			rstring[i] = *(curr_dev->ramdisk + *ppos- i) ;

		nbytes = bytes_to_do - copy_to_user(buf, rstring, bytes_to_do);
		kfree(rstring);
...
{	
	struct dev_area *curr_dev = file->private_data;
	int i,nbytes, maxbytes, bytes_to_do;
	char *temp,*x;
	...
	if(curr_dev->seek_dir == REGULAR)
	{
		...
		nbytes = bytes_to_do - copy_from_user(curr_dev->ramdisk + *ppos, buf, bytes_to_do);
		*ppos += nbytes;
	...
	else if(curr_dev->seek_dir == REVERSE)
	{
		...
		if (bytes_to_do == 0)
			pr_info("Reached start of the device on a write");

		temp = (char *) (kmalloc(bytes_to_do*sizeof(char),GFP_KERNEL));
		nbytes = bytes_to_do - copy_from_user(temp, buf, bytes_to_do);
			
		pr_info("read reverse temp = %s nbytes= %d",temp,nbytes);

		for(i=0;i<nbytes;i++)
		{
			 x = curr_dev->ramdisk + *ppos -i;
			 *x = temp[i];
		}
...
static void __exit my_exit(void)
{
	...
	if(free_device_list())
		pr_info("Deleted the linked list memory");
...
MODULE_AUTHOR("sai raghu vamsi");
MODULE_DESCRIPTION("assignmnet 4");
MODULE_LICENSE("GPL");
